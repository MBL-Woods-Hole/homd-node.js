...
router.get("/dld_taxtable/:type", function dld_taxtable(req, res) {
  ...
  let file_filter_txt =
    "HOMD.org Taxon Data::Site/Status Filter Applied" + " Date: " + dt.today;
...
router.post("/anno_search_data", async (req, res) => {
  ...
  if (anno.toUpperCase() === "BAKTA") {
    if (format === "fasta_aa") {
      ...
      q +=
        " from `BAKTA_sub_prokka`.orf WHERE core_ID in (" + unique_pidlst + ")";
      head_text_array = [
        "Genome_ID",
        "HMT-ID",
        "Domain",
...
router.post("/anno_data_by_gid", async (req, res) => {
  ...
  if (format.slice(0, 5) === "fasta") {
    ...
    if (type === "browser") {
      ...
      let fname =
        "HOMD_annotations" + anno_cap + dt.today + "_" + dt.seconds + ext;
      res.set({ "Content-Disposition": "attachment; filename=" + fname });
    } else if (type === "excel") {
      let fname =
        "HOMD_annotations" + anno_cap + dt.today + "_" + dt.seconds + ".xls";
      res.set({ "Content-Disposition": "attachment; filename=" + fname });
    } else {
      // error
      logger.error("Download table format ERROR");
      ...
      if (otids[n] in C.taxon_lineage_lookup) {
        for (let m in C.ranks) {
          ...
          if (rank == "subspecies") {
            ...
            txt +=
              C.taxon_lineage_lookup[otids[n]]["species"] + "\t" + cnts + "\t";
          } else {
            old_lineage = lineage + ";";
