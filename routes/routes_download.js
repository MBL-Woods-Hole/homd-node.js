...
router.get("/dld_taxtable_all/:type/", function dld_taxtable_all(req, res) {
    ...
    if (type === "browser") {
        ...
        res.set({
            "Content-Disposition":
                'attachment; filename="HOMD_taxon_table' +
                dt.today +
                "_" +
                dt.seconds +
                '.txt"',
...
router.get("/dld_refseqtable_all/:type", function dld_refseqtable(req, res) {
    ...
    for (let n in refseq_array) {
        ...
        tableTsv +=
            hmt +
            "\t" +
            refseq_array[n].refseq_id +
            "\t" +
            refseq_array[n].species +
            "\t" +
            refseq_array[n].length +
            "\t" +
            refseq_array[n].seqid_count +
            "\t" +
            refseq_array[n].seqids +
            "\n";
    }
...
router.get("/dld_taxtable/:type", function dld_taxtable(req, res) {
    ...
    send_list = helpers_taxa.apply_ttable_filter(
        req,
        req.session.ttable_filter,
    );
    let file_filter_txt =
        "HOMD.org Taxon Data::Site/Status Filter Applied" +
        " Date: " +
        dt.today;
...
router.get("/dld_tax_table/:version/", function dld_tax_table(req, res) {
    ...
    if (version === "V31") {
        let fullpath = path.join(
            ...
            "HOMD_taxon_tableV31.xls",
        );
        res.download(fullpath);
    } else if (version === "V42") {
        let fullpath = path.join(
            ENV.PATH_TO_STATIC_DOWNLOADS,
            "HOMD_taxon_tableV42.xls",
        );
        res.download(fullpath);
    } else {
...
router.get("/dld_taxabund/:type/:source/", function dld_taxabund(req, res) {
    ...
    if (source === "mapping") {
        if (type === "samples_file") {
            let fullpath = path.join(
                ENV.PATH_TO_STATIC_DOWNLOADS,
                "HOMD_AbundanceData_Mapping.tar.gz",
            );
            res.download(fullpath);
            return;
        }
        ...
        if (type === "samples_file") {
            let fullpath = path.join(
                ENV.PATH_TO_STATIC_DOWNLOADS,
                "eren2014_v1v3_rank_abundance_sums_homd.csv.gz",
            );
            res.download(fullpath);
            return;
        }
        ...
        if (type === "samples_file") {
            let fullpath = path.join(
                ENV.PATH_TO_STATIC_DOWNLOADS,
                "eren2014_v3v5_rank_abundance_sums_homd.csv.gz",
            );
            res.download(fullpath);
            return;
        }
        ...
        if (type === "samples_file") {
            let fullpath = path.join(
                ENV.PATH_TO_STATIC_DOWNLOADS,
                "HMP_16SRefseq_Ranked_Abundance_v1v3_homd.tar.gz",
            );
            res.download(fullpath);
            return;
        }
        header +=
            "HOMD Abundance Data from HMP 16S RefSeq V1-V3 (unpublished); ";
        abundance_order = C.hmp_refseq_abundance_order;
    } else if (source === "hmprefseqv3v5") {
        if (type === "samples_file") {
            let fullpath = path.join(
                ENV.PATH_TO_STATIC_DOWNLOADS,
                "HMP_16SRefseq_Ranked_Abundance_v3v5_homd.tar.gz",
            );
            res.download(fullpath);
            return;
        }
    ...
    for (let tax_string in C.abundance_lookup) {
        if (
            source === "mapping" &&
            Object.hasOwn(C.abundance_lookup[tax_string], "hmp_mapping") &&
            Object.keys(C.abundance_lookup[tax_string]["hmp_mapping"]).length >
                0
        ) {
            table_tsv +=
                tax_string + "\t" + C.abundance_lookup[tax_string]["otid"];
            ...
            source === "dewhirst" &&
            Object.hasOwn(C.abundance_lookup[tax_string], "dewhirst") &&
            ...
            source === "erenv1v3" &&
            Object.hasOwn(C.abundance_lookup[tax_string], "eren_v1v3") &&
            ...
            source === "erenv3v5" &&
            Object.hasOwn(C.abundance_lookup[tax_string], "eren_v3v5") &&
            ...
            source === "hmpmeta" &&
            Object.hasOwn(C.abundance_lookup[tax_string], "hmp_metaphlan") &&
            Object.keys(C.abundance_lookup[tax_string]["hmp_metaphlan"])
                .length > 0
        ) {
            table_tsv +=
                tax_string + "\t" + C.abundance_lookup[tax_string]["otid"];
            ...
            source === "hmprefseqv1v3" &&
            Object.hasOwn(C.abundance_lookup[tax_string], "hmp_refseq_v1v3") &&
            Object.keys(C.abundance_lookup[tax_string]["hmp_refseq_v1v3"])
                .length > 0
        ) {
            table_tsv +=
                tax_string + "\t" + C.abundance_lookup[tax_string]["otid"];
            ...
            source === "hmprefseqv3v5" &&
            Object.hasOwn(C.abundance_lookup[tax_string], "hmp_refseq_v3v5") &&
            Object.keys(C.abundance_lookup[tax_string]["hmp_refseq_v3v5"])
                .length > 0
        ) {
            table_tsv +=
                tax_string + "\t" + C.abundance_lookup[tax_string]["otid"];
...
///////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
router.get(
    "/dld_genome_table_all/:type/:filter",
    async function dld_genome_table_all(req, res) {
        let dt = helpers.get_today_obj();
...
router.get("/dld_genome_table/:type", function dld_genome_table(req, res) {
    ...
    let sendList = helpers_genomes.apply_gtable_filter(req);
    ...
    const tableTsv = create_genome_table(
        listOfGids,
        "table",
        type,
        fileFilterText,
...
// /////////////////////////////
router.get(
    "/dld_peptide_table_all/:study",
    function dld_peptide_table_all(req, res) {
        const study = req.params.study;
        let fullpath = path.join(
            ENV.PATH_TO_STATIC_DOWNLOADS,
            "homd_protein-peptides_study" + study + ".csv",
        );
        res.download(fullpath);
        return;
    },
...
router.post("/anno_search_data", async (req, res) => {
    ...
    if (anno.toUpperCase() === "BAKTA") {
        if (format === "fasta_aa") {
            ...
            q +=
                " a.attribute_locus_tag,' | ',start,'..',end,' | length:',b.length_aa) AS defline,";
            q += " UNCOMPRESS(seq_compressed) as sequence";
            q += " FROM BAKTA.gff a";
            q +=
                " JOIN BAKTA.faa b  on a.genome_id=b.genome_id and b.protein_id=a.attribute_locus_tag";
            ...
            q =
                "SELECT genome_id as gid,core_contig_acc as contig,core_ID as pid,core_start as start,core_end as end,bakta_Product as product,bakta_Gene as gene,bakta_Length as length,";
            q +=
                " bakta_EC,bakta_GO,bakta_COG,bakta_RefSeq,bakta_UniParc,bakta_UniRef";
            q +=
                " from `BAKTA_sub_prokka`.orf WHERE core_ID in (" +
                unique_pidlst +
                ")";
            head_text_array = [
                "Genome_ID",
        ...
        if (format === "fasta_aa") {
            ...
            q +=
                ...
                ".orf.genome_id,' | '," +
                anno_cap +
                ".orf.region,' | ',protein_id) AS defline,";
            q += " UNCOMPRESS(seq_compressed) AS sequence";
            q +=
                " from " +
                anno_cap +
                ".orf JOIN " +
                anno_cap +
                ".faa using(protein_id) WHERE protein_id in (" +
                unique_pidlst +
                ")";
            ...
            q +=
                ...
                ".orf JOIN " +
                anno_cap +
                ".ffn using(protein_id) WHERE protein_id in (" +
                unique_pidlst +
                ")";
            //q += " from "+anno_cap+".orf JOIN "+anno_cap+".ffn using(protein_id) WHERE protein_id in ('WKE52996.1','WKE52997.1','WKE52998.1')"
        } else {
            ...
            q =
                "SELECT genome_id as gid,region as acc,protein_id as pid,start,end,product,gene,length_aa as laa,";
            q +=
                "length_na as lna from " +
                anno_cap +
                ".orf WHERE protein_id in (" +
                unique_pidlst +
                ")";
    ...
    if (format.slice(0, 5) === "fasta") {
        ...
        ext = ".txt";
        result_text = create_anno_search_table(
            rows,
            anno,
            head_text_array,
            search_text,
        ...
        if (type === "browser") {
            ...
            let fname =
                "HOMD_search" + anno_cap + dt.today + "_" + dt.seconds + ext;
            res.set({ "Content-Disposition": "attachment; filename=" + fname });
        } else if (type === "excel") {
            let fname =
                "HOMD_search" + anno_cap + dt.today + "_" + dt.seconds + ".xls";
            res.set({ "Content-Disposition": "attachment; filename=" + fname });
        } else {
            // error
            logger.error("Download table format ERROR");
...
router.post("/anno_data_by_gid", async (req, res) => {
    ...
    q = "SELECT";
    q += " CONCAT('>',protein_id,' | ',genome_id,' | ',region) AS defline,";
    q += " UNCOMPRESS(seq_compressed) AS sequence";
    if (format === "fasta_aa" && anno === "ncbi") {
    ...
    } else if (format === "fasta_aa" && anno === "bakta") {
        q += " from BAKTA.faa WHERE BAKTA.faa.genome_id = '" + gid + "'";
        ...
        q =
            "SELECT region as acc,  gc, protein_id as pid, length_na,length_aa, `start`, `end`,";
        q +=
            " PROKKA.orf.product as product,PROKKA.orf.gene as gene,BAKTA_sub_prokka.orf.Bakta_product as bakta_product,BAKTA_sub_prokka.orf.Bakta_gene as bakta_gene";
        q += " FROM PROKKA.orf";
        q += " LEFT JOIN BAKTA_sub_prokka.orf on(protein_id=core_ID)";
        q += " WHERE PROKKA.orf.genome_id = '" + gid + "'";
    } else if (anno === "ncbi") {
        q =
            "SELECT region as acc,  gc, protein_id as pid, product, length_na,length_aa, `start`, `end`, gene";
        q += " FROM NCBI.orf";
...
router.post("/phage_search_data", async (req, res) => {
    ...
    if (format === "fasta") {
        ...
        ext = ".txt";
        result_text = create_phage_search_table(
            rows,
            head_text_array,
            search_text,
        );
        ...
        if (type === "browser") {
            ...
            let fname =
                "HOMD_phage_search" + dt.today + "_" + dt.seconds + ".xls";
            res.set({ "Content-Disposition": "attachment; filename=" + fname });
        } else {
            // error
            logger.error("Download table format ERROR");
...
router.get("/dld_phage_table/:type", async function dld_phage_table(req, res) {
    ...
    fileFilterText =
        "HOMD.org Phage Data:: All HOMD Data;" + " Date: " + dt.today;
    ...
    if (type === "browser") {
        ...
        res.set({
            "Content-Disposition":
                'attachment; filename="HOMD_phage_table' +
                dt.today +
                "_" +
                dt.seconds +
                '.txt"',
...
router.get(
    "/dld_crispr_table/:type",
    async function dld_crispr_table(req, res) {
        let dt = helpers.get_today_obj();
        const type = req.params.type;
        ...
        fileFilterText =
            "HOMD.org Crispr-Cas Data:: All HOMD Data;" + " Date: " + dt.today;
        tableTsv = create_table_from_sql_query(mysqlrows, fileFilterText);
        // https://homd.org/jbrowse/?data=homd_V11.02_phage_1.2%2FGCA_938045525.1&loc=GCA_938045525.1%7CCALIAX010000009.1%3A1..30909&tracks=DNA%2Cprokka%2Cprokka_ncrna%2Cncbi%2Cncbi_ncrna%2Ccenote%2Cgenomad&highlight=
        ...
        if (type === "browser") {
            ...
            res.set({
                "Content-Disposition":
                    'attachment; filename="HOMD_crispr_cas_table' +
                    dt.today +
                    "_" +
                    dt.seconds +
                    '.txt"',
...
router.get("/dld_amr_table/:type", async function dld_crispr_table(req, res) {
    ...
    fileFilterText =
        "HOMD.org AMR Data:: All HOMD Data;" + " Date: " + dt.today;
    tableTsv = create_table_from_sql_query(mysqlrows, fileFilterText);
    // https://homd.org/jbrowse/?data=homd_V11.02_phage_1.2%2FGCA_938045525.1&loc=GCA_938045525.1%7CCALIAX010000009.1%3A1..30909&tracks=DNA%2Cprokka%2Cprokka_ncrna%2Cncbi%2Cncbi_ncrna%2Ccenote%2Cgenomad&highlight=
    ...
    if (type === "browser") {
        ...
        res.set({
            "Content-Disposition":
                'attachment; filename="HOMD_AMR_table' +
                dt.today +
                "_" +
                dt.seconds +
                '.txt"',
...
router.get("/version/:index", function dld_pg(req, res) {
    ...
    if (idx === "01") {
        fullpath = path.join(
            ENV.PATH_TO_STATIC_DOWNLOADS,
            "HOMD_Full_Taxonomy2025_02_18_JMWv4_ForAddDropNameChange.xlsx",
        );
    } else {
        return;
    }
...
router.post(
    ...
    async function dld_fasta_post(req, res) {
...
function create_phage_search_table(sql_rows, header_array, search_term) {
    ...
    for (let n in sql_rows) {
        ...
        if (sql_rows[n].seq) {
            ...
            text +=
                "\t" + sql_rows[n].predictor + "\t" + sql_rows[n].seq_length;
...
async function stream_sqlquery_download(q, fname, res, format, type) {
    ...
    const stream = await queries.run_query_stream_async(q, res);
    ...
    if (anno === "prokka") {
        ...
        for (let n in sql_rows) {
            text +=
                ...
                sql_rows[n].pid +
                "\t" +
                sql_rows[n].length_na +
                "\t" +
                sql_rows[n].length_aa;
            text +=
                "\t" +
                sql_rows[n].start +
                "\t" +
            ...
            text +=
                "\t" +
                sql_rows[n].bakta_product +
                "\t" +
                sql_rows[n].bakta_gene;
            text += "\n";
        }
    } else {
        // anno === ncbi
...
function create_anno_search_table(sql_rows, anno, headers, search_term) {
    ...
    if (anno === "bakta") {
        ...
        for (let n in sql_rows) {
            hmt = helpers.make_otid_display_name(
                C.genome_lookup[sql_rows[n].gid].otid,
            );
            strain = C.genome_lookup[sql_rows[n].gid].strain;
            lineage =
                C.taxon_lineage_lookup[C.genome_lookup[sql_rows[n].gid].otid];
            text += sql_rows[n].gid;
            text += "\t" + hmt;
            text += "\t" + lineage.domain;
            text += "\t" + lineage.phylum;
            ...
            text +=
                ...
                sql_rows[n].bakta_GO +
                "\t" +
                sql_rows[n].bakta_COG;
            text +=
                "\t" +
                sql_rows[n].bakta_RefSeq +
                "\t" +
                sql_rows[n].bakta_UniParc +
                "\t" +
                sql_rows[n].bakta_UniRef;
            text += "\n";
        }
    } else {
        //text='anno table: '+anno
...
function create_taxon_table(otids, source, type, head_txt) {
    ...
    if (source === "table") {
        ...
        for (let n in otids) {
            ...
            if (C.dropped_taxids.indexOf(otid) != -1) {
                ...
                if (o2.domain) {
                    ...
                    let sites =
                        obj5[otid].major_body_site +
                        " (" +
                        obj5[otid].major_site_abundance +
                        ")";
                    if (obj5[otid].secondary_sites != "none assigned") {
                        sites =
                            sites +
                            " | " +
                            obj5[otid].secondary_sites +
                            " (" +
                            obj5[otid].secondary_site_abundance +
                            ")";
                    }
        ...
        for (let n in otids) {
            ...
            if (otids[n] in C.taxon_lineage_lookup) {
                txt +=
                    otid_pretty +
                    "\t" +
                    C.taxon_lineage_lookup[otids[n]].domain;
        ...
        for (let n in otids) {
            ...
            if (otids[n] in C.taxon_lineage_lookup) {
                for (let m in C.ranks) {
                    ...
                    lineage =
                        old_lineage + C.taxon_lineage_lookup[otids[n]][rank];
                    //logger.info(rank,lineage)
                    cnts = C.taxon_counts_lookup[lineage].taxcnt;
                    if (rank == "subspecies") {
                        txt +=
                            C.taxon_lineage_lookup[otids[n]]["subspecies"] +
                            "\t" +
                            otid_pretty; // no counts
                        ...
                        txt +=
                            C.taxon_lineage_lookup[otids[n]]["species"] +
                            "\t" +
                            cnts +
                            "\t";
                    } else {
                        old_lineage = lineage + ";";
                        txt +=
                            C.taxon_lineage_lookup[otids[n]][rank] +
                            "\t" +
                            cnts +
                            "\t";
                    }
        ...
        headers = [
            ...
            "Species_Seq_Count",
            "Species_Clone_Count",
            "Subspecies_Taxon_Count",
            "Subspecies_Seq_Count",
            "Subspecies_Clone_Count",
        ];
        txt += headers.join("\t") + "\n";
        for (let n in otids) {
            otid_pretty = helpers.make_otid_display_name(otids[n]);
