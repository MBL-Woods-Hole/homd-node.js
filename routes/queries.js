...
export const get_gtdb_tax = (genomes) => {
  let g = genomes.join("','");
  let q =
    "SELECT genome_id, GTDB_taxonomy  from `" + C.genomes_table_name + "`";
...
export const get_taxon_info_query = (otid) => {
  let q =
    "SELECT notes,`general`,prevalence as prev,cultivability as culta,disease_associations as disease,phenotypic_characteristics as pheno";
  q += " from taxon_info";
  q += " WHERE otid='" + otid + "'";
  return q;
};
...
export const get_dropped_taxa = () => {
  let q =
    "SELECT otid, naming_status, cultivation_status, notes, genus, species from otid_prime";
...
export const get_annotation_query = (gid, anno) => {
  ...
  if (anno === "prokka") {
    q =
      "SELECT a.region, type, gc, a.protein_id, product, c.length_na, b.length_aa, `start`, `end`, gene";
    q += " FROM PROKKA.orf_gff a";
    q +=
      " LEFT JOIN PROKKA.faa b on a.genome_id=b.genome_id and a.protein_id=b.protein_id ";
    q +=
      " LEFT JOIN PROKKA.ffn c on a.genome_id=c.genome_id and a.protein_id=c.protein_id ";
    ...
    q +=
      " LEFT JOIN NCBI.faa b on a.genome_id=b.genome_id and a.protein_id=b.protein_id ";
    q +=
      " LEFT JOIN NCBI.ffn c on a.genome_id=c.genome_id and a.protein_id=c.protein_id ";
    q += " WHERE a.genome_id = '" + gid + "'";
  } else {
    // BAKTA
    ...
    q =
      'SELECT a.region, a.type,  a.attribute_locus_tag as protein_id,  a.attribute_product as product, "c.length_na", b.length_aa, a.`start`, a.`end`, a.attribute_gene as gene ';
    q += " FROM BAKTA.gff a";
    q +=
      " LEFT JOIN BAKTA.faa b on a.genome_id=b.genome_id and b.protein_id=a.attribute_locus_tag";
...
export const get_all_pangenomes_query = () => {
  let q =
    "SELECT pangenome_name,homd_genome_version,description FROM pangenomes";
  q +=
    " WHERE active='1' AND homd_genome_version = '" +
    C.genomic_refseq_version +
    "'";
...
export const get_peptide = () => {
  /// USING Ver 3.1
  let q =
    "SELECT `genomes`.otid, study_id, seq_id, organism, protein_accession,jb_link,molecule,peptide_id,peptide,product from protein_peptide";
...
export const get_peptide2 = () => {
  /// USING Ver 3.1
  let q =
    "SELECT seq_id, `genomes`.otid, organism, protein_count, peptide_count,study_id from protein_peptide_counts ";
  q += " JOIN `genomes` using (seq_id)";
...
export const get_crispr_cas_data = (gid) => {
  let q =
    "SELECT contig,operon,operon_pos,prediction,crisprs,distances,prediction_cas,prediction_crisprs";
  q += " FROM crispr_cas where genome_id='" + gid + "'";
  return q;
};
export const get_all_crispr_cas_data = () => {
  let q =
    "SELECT genome_id,contig,operon,operon_pos,prediction,crisprs,distances,prediction_cas,prediction_crisprs";
  q += " FROM crispr_cas";
  return q;
};
export const get_amr_data = (gid) => {
  let q =
    "SELECT homd.amr.protein_id,element_symbol,element_name,scope,type,subtype,class,";
  q +=
    "subclass,method,target_length,ref_seq_length,pct_cov_of_ref,pct_ident_to_ref,align_length,closest_ref_acc, ";
  q += "closest_ref_name,hmm_acc,hmm_description,region,start,end";
  q += " FROM homd.amr";
...
export const get_all_amr_data = () => {
  let q =
    "SELECT amr.genome_id,homd.amr.protein_id,element_symbol,element_name,scope,type,subtype,class,";
  q +=
    "subclass,method,target_length,ref_seq_length,pct_cov_of_ref,pct_ident_to_ref,align_length,closest_ref_acc, ";
  q += "closest_ref_name,hmm_acc,hmm_description,region,start,end";
  q += " FROM homd.amr";
...
export const get_all_genomes = () => {
  // for downld all
  let q = genome_query();
  return q;
};

export const get_genome = (gid) => {
...
export const get_contigs = (gid) => {
  // always NCBI for taxon description
  //const db = 'NCBI_' + gid
  let q =
    "SELECT region, GC from `NCBI`.`molecule` WHERE genome_id = '" + gid + "'";
  return q;
};

export const get_contig = (gid, mid) => {
  // always NCBI for taxon description
  //const db = 'NCBI_' + gid
  let q =
    "SELECT UNCOMPRESS(seq_compressed) as seq from `NCBI`.`contig` WHERE genome_id = '" +
    gid +
    "' and region='" +
    mid +
    "'";
...
export const get_all_phage_for_download = () => {
  let q =
    "SELECT genome_id,site,cenote_count,cenote_coverage_bps,cenote_coverage_pct,";
...
export const get_phage = (gid) => {
  // always NCBI for taxon description

  let q =
    "SELECT phage_id,type,contig,start,end FROM phage_data where genome_id='" +
    gid +
    "'";
  return q;
};
...
export const get_phage_fasta = (search_id_list) => {
  //
  let q =
    "SELECT CONCAT('>',search_id,' | ',genome_id,' | ',contig,' | ',predictor,' | ',start,'..',end,";
  q += "' | ',accession,";
...
export const get_phage_from_ids_noseqs = (search_id_list) => {
  // always NCBI for taxon description
