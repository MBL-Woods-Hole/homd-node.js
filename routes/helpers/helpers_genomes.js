...
export const get_taxa_wgenomes = () => {
  let alltax_list = Object.values(C.taxon_lookup).filter(
  ...
  for (let item in req.body) {
    ...
    for (let item in cat_array) {
      if (Object.prototype.hasOwnProperty.call(req.body, cat_array[item])) {
        req.session.gtable_filter.level[cat_array[item]] =
          req.body[cat_array[item]];
      }
    }
    // Tax Status
    let status_array = [
    ...
    for (let item in status_array) {
      if (Object.prototype.hasOwnProperty.call(req.body, status_array[item])) {
        req.session.gtable_filter.status[status_array[item]] =
          req.body[status_array[item]];
      }
    }
    // Tax Site
    let site_array = [
    ...
    for (let item in site_array) {
      if (Object.prototype.hasOwnProperty.call(req.body, site_array[item])) {
        req.session.gtable_filter.site[site_array[item]] =
          req.body[site_array[item]];
      }
    }
    // Tax Abundance
    let abund_array = [
    ...
    for (let item in abund_array) {
      if (Object.prototype.hasOwnProperty.call(req.body, abund_array[item])) {
        req.session.gtable_filter.abund[abund_array[item]] =
          req.body[abund_array[item]];
      }
    }
  }
};
...
export const apply_species = (lst) => {
  ...
  for (let i in lst) {
    ...
    lst[i].subspecies = "";
    if (
      Object.hasOwn(C.taxon_lineage_lookup, otid) &&
...
export const on_paging = (glist, fltr, pd) => {
  ...
  if (cbp > sendList.length) {
    ...
    txt =
      "page: <span class='gray'>" +
      pageData.page +
      " (of " +
      pageData.number_of_pages +
      "p)</span>" +
      space;
    ...
    txt +=
      space +
      "Jump to Page: <select class='gray' onchange=\"document.location.href='genome_table?page='+this.value\" >";
    ...
    txt +=
      "</select>" +
      space +
      "(<a href='genome_table?page=1'>Return to Page1</a>)";
  }
...
export const apply_pages = (glist, fltr, pd) => {
  ...
  if (trows > pd.rows_per_page) {
    ...
    if (pd.page > pd.number_of_pages) {
      pd.page = 1;
    }
    if (pd.page < 1) {
      pd.page = pd.number_of_pages;
    }
    ...
    if (pd.show_page === 1) {
      ...
      genomeList = glist.slice(
        pd.rows_per_page * (pd.show_page - 1),
        pd.rows_per_page * pd.show_page,
      ); // second 200
...
export const apply_gtable_filter = (req) => {
  ...
  if (vals.text.txt_srch) {
    big_g_list = get_filtered_genome_list(
      big_g_list,
      vals.text.txt_srch,
      vals.text.field,
    );
  ...
  //letter
  if (vals.letter && vals.letter.match(/[A-Z]{1}/)) {
    // always caps
    //helpers.print(['FILTER::GOT a TaxLetter: ',vals.letter])
    // COOL.... filter the whole list
    big_g_list = big_g_list.filter(
      (item) => item.organism.toUpperCase().charAt(0) === vals.letter,
    );
  }
  //phylum
  if (vals.phylum !== "") {
  ...
  // Assembly Level
  let level_on = Object.keys(vals.level).filter(
    (item) => vals.level[item] === "on",
  );
  //logger.info('vals',vals)
  //logger.info('level_on',level_on)
  big_g_list = big_g_list.filter(function (item) {
    if (
      level_on.includes(helpers.getKeyByValue(C.genome_level_all, item.level))
    ) {
      return item;
    }
  });
  ...
  // ADV::Tax Status ////////////////////////////////////////////////
  let status_on = Object.keys(vals.status).filter(
    (item) => vals.status[item] === "on",
  );
  ...
  big_g_list = big_g_list.filter(function (item) {
    if (status_on.length === default_length_of_status) {
      ...
      if (nstatus == "phylotype") {
        status = "phylotype";
      } else if (nstatus.slice(0, 5) === "named") {
        // this catches named** and name NVP
        status = "named_" + cstatus;
      } else if (nstatus.slice(0, 7) === "unnamed") {
        status = "unnamed_" + cstatus;
      } else {
        logger.warn(
          "XXXX WARNING:: nstatus: " + nstatus + " cstatus: " + cstatus,
        );
      }
  ...
  // ADV::Tax Sites ////////////////////////////////////////////////
  let site_on = Object.keys(vals.site).filter(
    (item) => vals.site[item] === "on",
  );
  ...
  // ADV::Tax Abundance ////////////////////////////////////////////////
  //logger.info('vals',vals)
  let abund_on = Object.keys(vals.abund).filter(
    (item) => vals.abund[item] === "on",
  );
  ...
  big_g_list = big_g_list.filter(function filterAbundance(item) {
    ...
    if (abund_on.length === default_length_of_abund) {
      ...
      if (
        Object.hasOwn(C.site_lookup, item.otid) &&
        C.site_lookup[item.otid].major_body_site
      ) {
        ...
        for (let n in abund_on) {
          let high_to_scarce = helpers.capitalizeFirst(
            abund_on[n].split("_")[0],
          );
...
export const get_filtered_genome_list = (
  gidObjList,
  searchText,
  searchField,
) => {
  ...
  if (searchField === "strain") {
    sendList = gidObjList.filter((item) =>
      item.strain.toLowerCase().includes(searchText),
    );
  } else {
    ...
    tmpSendList = gidObjList.filter((item) =>
      item.gid.toLowerCase().includes(searchText),
    );
    for (let n in tmpSendList) {
      tempObj[tmpSendList[n].gid] = tmpSendList[n];
    }
    ...
    if (searchText.slice(0, 3) === "hmt") {
      let pototid = parseInt(
        searchText.slice(3).replace("-", "").replace("_", ""),
      );
      tmpSendList = gidObjList.filter((item) => {
      ...
      tmpSendList = gidObjList.filter((item) =>
        item.otid.toString().toLowerCase().includes(searchText),
      );
    }
    ...
    //Species
    tmpSendList = gidObjList.filter((item) =>
      item.organism.toLowerCase().includes(searchText),
    );
...
export const get_checkm_status = (ginfo) => {
  ...
  if (!ginfo.checkM_completeness) {
    ginfo.checkM_completeness = 0;
  }
  if (!ginfo.checkM2_completeness) {
    ginfo.checkM2_completeness = 0;
  }
  if (ginfo.checkM_completeness == 0 && ginfo.checkM2_completeness == 0) {
    //pct_dif_cont = 0;
  } else {
    let dif1 = Math.abs(
      parseFloat(ginfo.checkM_completeness) -
        parseFloat(ginfo.checkM2_completeness),
    );
    let avg1 =
      (parseFloat(ginfo.checkM_completeness) +
        parseFloat(ginfo.checkM2_completeness)) /
  ...
  if (!ginfo.checkM_contamination) {
    ginfo.checkM_contamination = 0;
  }
  if (!ginfo.checkM2_contamination) {
    ginfo.checkM2_contamination = 0;
  }
  if (ginfo.checkM_contamination == 0 && ginfo.checkM2_contamination == 0) {
    pct_dif_cont = 0;
  } else {
    let dif2 = Math.abs(
      parseFloat(ginfo.checkM_contamination) -
        parseFloat(ginfo.checkM2_contamination),
    );
    let avg2 =
      (parseFloat(ginfo.checkM_contamination) +
        parseFloat(ginfo.checkM2_contamination)) /
