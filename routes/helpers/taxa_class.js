  ...
  for (let i = 0, len = taxonomy_obj.length; i < len; i++) {
    ...
    for (let field_name in in_obj) {
      ...
      if (is_rank) {
        ...
        if (Object.hasOwn(in_obj, taxa_rank)) {
          let taxa_name = in_obj[taxa_rank];
          if (taxa_name) {
            let node = get_by_key(
              dictMap_by_name_n_rank,
              taxa_name + "_" + taxa_rank,
            );
            ...
            if (node) {
              ...
              current_dict = make_current_dict(
                taxa_name,
                taxa_rank,
                i_am_a_parent,
                taxon_name_id,
              ...
              add_to_dict_by_key(
                dictMap_by_name_n_rank,
                current_dict.taxon + "_" + current_dict.rank,
                current_dict,
              );

              add_to_dict_by_key(
                dictMap_by_otid_n_rank,
                current_dict.otid + "_" + current_dict.rank,
                current_dict,
              );
