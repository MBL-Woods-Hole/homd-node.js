...
export const readFromFile = (file, ext) =>
...
export const filter_for_phylum = (obj_list, phylum) => {
    ...
    let new_obj_list = obj_list.filter((item) => {
        if (
            Object.hasOwn(C.taxon_lineage_lookup, item.otid) &&
            C.taxon_lineage_lookup[item.otid].phylum == phylum
        ) {
            return true;
        }
    });
...
export const calculate_homd_stats = () => {
    ...
    let homd_dropped_taxa = Object.values(C.taxon_lookup).filter(
        (item) => item.active_status.toLowerCase() === "dropped",
    );
    let homd_reference_taxa = Object.values(C.taxon_lookup).filter(
        (item) => item.active_status.toLowerCase() === "reference",
    );
    s.ref_count = homd_reference_taxa.length;
    s.dropped_count = homd_dropped_taxa.length;
    // Status
    s.taxon_count = homd_taxa.length;
    count = homd_taxa.filter(
        (item) =>
            item.naming_status.toLowerCase().slice(0, 5) === "named" &&
            item.cultivation_status.toLowerCase().slice(0, 4) === "cult",
    ).length;
    ...
    count = homd_taxa.filter(
        (item) =>
            item.naming_status.toLowerCase().slice(0, 5) === "named" &&
            item.cultivation_status.toLowerCase().slice(0, 6) === "uncult",
    ).length;
    pct = (count / s.taxon_count) * 100;
    ...
    count = homd_taxa.filter(
        (item) =>
            item.naming_status.toLowerCase().slice(0, 7) === "unnamed" &&
            item.cultivation_status.toLowerCase().slice(0, 4) === "cult",
    ).length;
    pct = (count / s.taxon_count) * 100;
    ...
    count = homd_taxa.filter(
        (item) => item.naming_status.toLowerCase().slice(0, 9) === "phylotype",
    ).length;
    pct = (count / s.taxon_count) * 100;
    ...
    count = homd_taxa.filter(
        (item) => item.body_site === "Environmental",
    ).length;
    pct = (count / s.taxon_count) * 100;
    ...
    C.homd_taxonomy.taxa_tree_dict_map_by_rank["phylum"].map((taxitem) => {
        //logger.info(taxitem)
        if (
            !Object.hasOwn(C.taxon_counts_lookup, "Bacteria;" + taxitem.taxon)
        ) {
    ...
    count = homd_genomes.filter(
        (item) => item.level.slice(0, 8) === "Complete",
    ).length;
    pct = (count / s.genome_count) * 100;
    s.genome_assembly_complete = {
        count: count,
        pct_of_genomes: pct.toFixed(1),
    };
    count = homd_genomes.filter(
        (item) => item.level.slice(0, 6) === "Contig",
    ).length;
    pct = (count / s.genome_count) * 100;
    s.genome_assembly_contig = { count: count, pct_of_genomes: pct.toFixed(1) };
    count = homd_genomes.filter(
        (item) => item.level.slice(0, 8) === "Scaffold",
    ).length;
    pct = (count / s.genome_count) * 100;
    s.genome_assembly_scaffold = {
        count: count,
        pct_of_genomes: pct.toFixed(1),
    };
    count = homd_genomes.filter(
        (item) => item.level.slice(0, 10) === "Chromosome",
    ).length;
    pct = (count / s.genome_count) * 100;
    s.genome_assembly_chromosome = {
        count: count,
        pct_of_genomes: pct.toFixed(1),
    };
    let min_contigs = 1000,
        max_contigs = 0,
        min_length = 10000000,
        max_length = 0;
    ...
    count = homd_taxa
        .filter((item) => item.body_site === "Oral")
        .map((item) => item.genomes)
        .flat().length;
    pct = (count / s.genome_count) * 100;
    s.genome_site_oral = { count: count, pct_of_genomes: pct.toFixed(1) };

    count = homd_taxa
        .filter((item) => item.body_site === "Nasal")
        .map((item) => item.genomes)
        .flat().length;
    pct = (count / s.genome_count) * 100;
    ...
    count = homd_taxa
        .filter((item) => item.body_site === "Skin")
        .map((item) => item.genomes)
        .flat().length;
    pct = (count / s.genome_count) * 100;
    ...
    count = homd_taxa
        .filter((item) => item.body_site === "Gut")
        .map((item) => item.genomes)
        .flat().length;
    pct = (count / s.genome_count) * 100;
    ...
    count = homd_taxa
        .filter((item) => item.body_site === "Vaginal")
        .map((item) => item.genomes)
        .flat().length;
    pct = (count / s.genome_count) * 100;
    ...
    count = homd_taxa
        .filter((item) => item.body_site === "Pathogen")
        .map((item) => item.genomes)
        .flat().length;
    pct = (count / s.genome_count) * 100;
    ...
    count = homd_taxa
        .filter((item) => item.body_site === "Environmental")
        .map((item) => item.genomes)
        .flat().length;
    pct = (count / s.genome_count) * 100;
    ...
    count = homd_taxa
        .filter((item) => item.body_site === "Reference")
        .map((item) => item.genomes)
        .flat().length;
    pct = (count / s.genome_count) * 100;
    ...
    count = homd_taxa
        .filter((item) => item.body_site === "Unassigned")
        .map((item) => item.genomes)
        .flat().length;
    pct = (count / s.genome_count) * 100;
    ...
    count = homd_annos.filter(
        (item) => Object.keys(item.ncbi).length != 0,
    ).length;
    pct = (count / s.genome_count) * 100;
    s.genome_ncbi = { count: count, pct_of_genomes: pct.toFixed(1) };
    count = homd_annos.filter(
        (item) => Object.keys(item.prokka).length != 0,
    ).length;
    pct = (count / s.genome_count) * 100;
    s.genome_prokka = { count: count, pct_of_genomes: pct.toFixed(1) };
    count = homd_annos.filter(
        (item) => Object.keys(item.bakta).length != 0,
    ).length;
    pct = (count / s.genome_count) * 100;
    ...
    homd_phage.map((item) => {
        ...
        s.phage.count +=
            parseInt(item.cenote_count) + parseInt(item.genomad_count);
    });
...
export const logPoolStatus = (pool, sql) => {
    ...
    let obj = {
        connectlimit: pool.pool.config.connectionLimit,
