...
module.exports.update_user_info = function update_user_info(uid, new_info) {
    var updateQuery = "UPDATE user set first_name='"+new_info.first_name+"', last_name='"+new_info.last_name+"', email='"+new_info.email+"', institution='"+new_info.institution+"' WHERE user_id='"+uid+"'"
    console.log(updateQuery)
    return updateQuery
}
module.exports.update_user_pw_admin = function update_user_pw_admin(uid, pw) {
    var updateQuery = "UPDATE user set encrypted_password=CONCAT('*', UPPER(SHA1(UNHEX(SHA1('"+pw+"'))))) WHERE user_id='"+uid+"'"
    console.log(updateQuery)
    return updateQuery
}
...
module.exports.get_all_users = function get_all_users() {
    var q = "SELECT * from user order by last_name"
    return q;
}
