...
function show_phage_hits(gid, contig, species, predictor, h) {
    ...
    html =
        ...
        " <i>" +
        species +
        '</i></span><a id="close_btn"  href="#" onclick="close_phage_info()">Close</a></div>';
    html += "<div id='phage-hit-results-div'>";
    html += "<table class='result-table sortable' id='phage-table2' >";
    html += "<thead>";
    html +=
        "<tr><th class='c1 sorttable_nosort'>Genome Viewer</th><th class='c1 sorttable_nosort'>View Sequence</th><th class='c2 sorttable_nosort'>Genome-ID</th><th class='c2 sorttable_nosort'>Contig</th><th class='c1 sorttable_nosort'>Predictor</th>";
    ...
    if (predictor === "bakta") {
        ...
        for (n in hits) {
            html += "<tr>";
            html +=
                "<td class='center'><a href='#' onclick=\"open_genome_viewer('" +
                hits[n][0] +
                "')\">link</a></td>";
            html +=
                "<td class='center'><a href='#' onclick=\"open_phage_sequence('" +
                hits[n][0] +
                "')\">open</a></td>";
            html +=
                ...
                hits[n][2] +
                "'>" +
                hits[n][2].toUpperCase() +
                "</td>";
            html +=
                "<td class='center' title='" +
                hits[n][3] +
                "'>" +
                hits[n][3].toUpperCase() +
                "</td>";
            html +=
                "<td class='center' title='" +
                hits[n][1] +
...
function show_anno_hits(obj, anno, gid) {
    ...
    var html =
        ...
        "</center>";
    html +=
        "<span style='float:right;'><a href='#' onclick=\"close_anno_info()\">close</a></span>";
    html += "<br><table class='result-table'>";
    html +=
        "<tr><th>Accession</th><th>Type</th><th>Protein-ID</th> <th>Genome Viewer</th><th>NA<br>length/seq</th><th>AA<br>length/seq</th><th>Gene</th><th>Gene Product</th></tr>";
    ...
    xmlhttp.open("POST", "advanced_anno_orf_search", true); // in routes_search.js
        ...
        if (xmlhttp.readyState == 4 && xmlhttp.status == 200) {
            ...
            for (n in resp_data) {
                ...
                let loc =
                    ...
                    locstop.toString();
                let highlight =
                    seqacc + ":" + start.toString() + ".." + end.toString();

                html += "<tr>";
                html +=
                    "<td><a href='https://www.ncbi.nlm.nih.gov/nuccore/" +
                    resp_data[n].acc +
                    "' target='_blank'>" +
                    resp_data[n].acc +
                    "</a></td>";
                ...
                if (anno === "ncbi") {
                    html +=
                        "<td><a href='https://www.ncbi.nlm.nih.gov/protein/" +
                        resp_data[n].pid +
                        "' target='_blank'>" +
                        resp_data[n].pid +
                        "</a></td>";
                    ...
                    let line =
                        "<a href='/genome/open_ftp_file?prokka_pid=" +
                        resp_data[n].pid +
                        "' target='_blank'>" +
                        resp_data[n].pid +
                        "</a>";
                ...
                html +=
                    " <a title='JBrowse/Genome Viewer:" +
                    gid +
                    "' href='#' onclick=\"open_jbrowse('" +
                    gid +
                    "','anno_table','','','" +
                    anno +
                    "','" +
                ...
                if (!resp_data[n].laa || resp_data[n].laa == "0") {
                    ...
                    html +=
                        " [<a title='Amino Acid' href='#' onclick=\"get_AA_NA_seq('aa','" +
                        resp_data[n].pid +
                        "','" +
                        db +
                        "','" +
